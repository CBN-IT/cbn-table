(function(scope) {
	 ...
	 * 
	 * - `column(column)`: you can use this object to pre-process the column object;
	...
	scope.DataTypes = {
		...
		"_": {
			 ...
			 */
			filters: {
				"exact": function(val, expr) {
					return val == expr;
				},
				"exact-ci": function(val, expr) {
		...
		"index": {
			value: function(item, column) { return column._index; }
		},
		...
		"date": { // requires Moment JS for parsing dates
			...
			display: function(val, column) { return val.format(column.dateFormat) }
		}
	};
	...
	scope.FilterBehavior = {
		...
		observers: [
			...
			"_filterOrSort(items.*, dataFilters.*, sortOrdering.*)"
		],
		...
		filter: function(dataFilters, ordering, options) {
			...
			var filteredItems;
			if (!this.items) {
				filteredItems = []; // nothing to do...
				
			} else if (scope.utils._isEmptyObject(dataFilters)) {
				// skip filtering...
		...
		_processColumns: function(columns) {
			...
			for (var i=0; i<columns.length; i++) {
				...
				this._processColumn(colDesc.obj);
		 ...
		 * 
		 * @param {Object} column The column descriptor to pre-process.
		 * @return {Object} The processed column's descriptor.
		 */
		_processColumn: function(column) {
			// infer defaults
			...
			column._f_value = dt.value || du.value;
			column._f_display = du.display || du.display;
			if (column.filter) {
			...
			column._f_compare = dt.compare || du.compare;
